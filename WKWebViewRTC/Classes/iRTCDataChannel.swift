...
class iRTCDataChannel : NSObject, RTCDataChannelDelegate {
	...
	init(
		...
		if self.rtcDataChannel == nil {
			NSLog("iRTCDataChannel#init() | rtcPeerConnection.createDataChannelWithLabel() failed")
			return
		}
		...
		self.eventListener!([
			...
			"channel": [
				...
				"readyState": iRTCDataChannel.stateToString(state: self.rtcDataChannel!.readyState),
	...
	func setListener(
		...
		NSLog("iRTCDataChannel#setListener()")
	...
	func sendString(
		...
		NSLog("iRTCDataChannel#sendString()")
		...
		if !result {
			NSLog("iRTCDataChannel#sendString() | RTCDataChannel#sendData() failed")
		}
	}
	...
	func sendBinary(
		...
		NSLog("iRTCDataChannel#sendBinary()")
		...
		let result = self.rtcDataChannel!.sendData(buffer)
		if !result {
			NSLog("iRTCDataChannel#sendBinary() | RTCDataChannel#sendData() failed")
		}
	}

	func close() {
		NSLog("iRTCDataChannel#close()")
	...
	func getState() -> String {
		return iRTCDataChannel.stateToString(state: self.rtcDataChannel!.readyState)
	}
	...
	func dataChannelDidChangeState(_ channel: RTCDataChannel) {
		...
		NSLog("iRTCDataChannel | state changed [state:%@]", String(describing: state_str))
	...
	func dataChannel(_ channel: RTCDataChannel, didReceiveMessageWith buffer: RTCDataBuffer) {
		if !buffer.isBinary {
			NSLog("iRTCDataChannel | utf8 message received")
			...
			NSLog("iRTCDataChannel | binary message received")
	...
	func dataChannel(_ channel: RTCDataChannel, didChangeBufferedAmount amount: UInt64) {
		NSLog("iRTCDataChannel | didChangeBufferedAmount %d", amount)
